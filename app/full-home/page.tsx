...
export default function FullHomePage() {
  ...
  useGSAP(
    () => {
      // 1. Configurar Parallax para el patrón de fondo
      gsap.to('.parallax-bg', {
        yPercent: 40,
        ease: 'none',
        scrollTrigger: {
      ...
      // Animación de la sección Pillars
      ...
      tlPillars.fromTo(
        '.pillar-grid',
        {
          backgroundColor: 'hsl(24, 10%, 85%)',
        },
        {
          backgroundColor: 'rgba(109, 72, 53, 0.2)',
          ease: 'none',
        },
        0,
      );
      ...
      tlPillars.fromTo(
        ...
        {
          color: (index, target) => {
            const card = (target as HTMLElement).closest('.pillar-container');
            const cards = gsap.utils.toArray('.pillar-container');
            const cardIndex = cards.indexOf(card!);
            return cardIndex === 2 ? '#3B281C' : 'rgba(229, 217, 202, 0.9)';
          },
          ease: 'none',
          stagger: 0.15,
        },
